...
class Lexicon:
    ...
    def __init__(self) -> None:
        """Load information into coupled word-index mappings and embedding matrix."""
        # FINISH THIS FUNCTION
        
        self.mappings = None
        self.embeddings = None

        # Store your stuff! Both the word-index mappings and the embedding matrix.
        #
    ...
    def from_file(cls, file: Path) -> Lexicon:
        ...
        with open(file) as f:
            ...
            for line in f:  # All of the other lines are regular.
                ...
                word = split_line[0]
                embeddings = ([float(i) for i in split_line[1:]])
                
                mappings.append(word)
                idx += 1
                
                embeddings_matrix.append(embeddings) #create embeddings matrix
            
            lexicon.embeddings = torch.tensor(embeddings_matrix)
            lexicon.mappings = Integerizer(mappings)
        return lexicon
    ...
    def find_similar_words(
        ...
        word_idx = Integerizer.index(self.mappings, word)

        e = self.embeddings

        row = torch.reshape(e[word_idx], (-1,))

        min_idx = None
        add_idx = None
        
        if (minus is not None) and (plus is not None):
            min_idx = Integerizer.index(self.mappings, minus)
            m = torch.reshape(e[min_idx], (-1,))

            add_idx = Integerizer.index(self.mappings, plus)
            a = torch.reshape(e[add_idx], (-1,))

            row = row - m + a

        similarities = (e * row).sum(dim=1) / (e.norm(dim=1) * row.norm()) 

        similarities[word_idx] = float("-inf")
        if min_idx is not None: 
            similarities[min_idx] = float("-inf")
        if add_idx is not None: 
            similarities[add_idx] = float("-inf")

        _, idx = torch.topk(similarities, k=10, largest=True)
        
        words = [self.mappings[i] for i in idx.tolist()]
