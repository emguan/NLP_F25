...
class ConditionalRandomField(HiddenMarkovModel):
    ...
    def updateAB(self) -> None:
        ...
        if self.unigram:
            A = self.WA[0].expand(self.k, self.k).clone()
        else:
            A = self.WA.clone()
        ...
        A[:, self.bos_t] = 0.0 # never transition INTO BOS
        A[self.eos_t, :] = 0.0 # never transition OUT OF EOS
        B[self.bos_t, :] = 0.0 # BOS never emits a real word
        B[self.eos_t, :] = 0.0 # EOS never emits a real word
