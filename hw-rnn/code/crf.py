...
class ConditionalRandomField(HiddenMarkovModel):
    ...
    def init_params(self) -> None:
        ...
        if self.unigram:
            self.WA = 0.01 * torch.randn(1, self.k)
        else:
            self.WA = 0.01 * torch.randn(self.k, self.k)
    ...
    def updateAB(self) -> None:
        ...
        if self.unigram and self.WA.size(0) == 1:
            A_logits = self.WA.repeat(self.k, 1)   # k x k, rows identical
        else:
            A_logits = self.WA                     # k x k

        B_logits = self.WB                         # k x V

        A = torch.exp(A_logits)
        B = torch.exp(B_logits)

        mask_A = torch.ones_like(A)
        mask_A[:, self.bos_t] = 0.0
        mask_A[self.eos_t, :] = 0.0

        mask_B = torch.ones_like(B)
        mask_B[self.bos_t, :] = 0.0
        mask_B[self.eos_t, :] = 0.0
    ...
    def logprob(self, sentence: Sentence, corpus: TaggedCorpus) -> TorchScalar:
        ...
        joint = super().logprob(sentence, corpus)
        normalizer = super().logprob(sentence.desupervise(), corpus)
    ...
    def logprob_gradient_step(self, lr: float) -> None:
        ...
        if self.unigram:
            grad_WA = self.A_counts.sum(dim=0, keepdim=True)   # 1 x k
            self.WA += lr * grad_WA
        else:
    ...
    def reg_gradient_step(self, lr: float, reg: float, frac: float):
        ...
        finite_WA = torch.isfinite(self.WA)
        finite_WB = torch.isfinite(self.WB)

        self.WA[finite_WA] *= decay
        self.WB[finite_WB] *= decay
