...
from typing import Counter as CounterType, Iterable, List, Optional, Dict, Tuple, Deque, Set
...
class EarleyChart:
    ...
    def _scan(self, item: Item, position: int) -> None:
        ...
        if position < len(self.tokens) and self.tokens[position] == item.next_symbol():
            ...
            if self._push(position + 1, new_item, score=self._score(position, item), bp=('scan', self.tokens[position])):
    ...
    def _attach(self, item: Item, position: int) -> None:
        ...
        for customer in self.cols[mid].all():  # could you eliminate this inefficient linear search?
            if customer.next_symbol() == item.rule.lhs:
                ...
                if self._push(position, new_item, score=new_cost, bp=('attach', customer, item)): 
    ...
    def _push(self, position: int, item: Item, score: float, bp=None) -> None:
        self.cols[position].push(item, score, bp)
...
class Agenda:
    ...
    def push(self, item: Item, score: float, bp: Optional[Tuple[str, object]] = None) -> None:
    ...
    def __repr__(self):
        ...
        return f"{self.__class__.__name__}({list(self._items)}; queue={list(self._todo)})"
