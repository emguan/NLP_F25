...
def main():
    ...
    if args.smoother == UNIFORM:
        ...
        if args.lexicon is None:
            parser.error(f"{args.smoother} requires a lexicon")
        lm = EmbeddingLogLinearLanguageModel(vocab = vocab, lexicon_file = args.lexicon, l2 = args.l2_regularization, epochs = args.epochs, lr = args.lr, device = args.device)
    elif args.smoother == IMPROVED:
        if args.lexicon is None:
            parser.error(f"{args.smoother} requires a lexicon")
        lm = ImprovedLogLinearLanguageModel(vocab = vocab, lexicon_file = args.lexicon, l2 = args.l2_regularization, epochs = args.epochs, lr = args.lr, device = args.device)
    else:
